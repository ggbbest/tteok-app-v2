    ...
    constructor() {
        ...
        Layout.current.content.append(this.container = el(".classic-view",
            el("section",
                ...
                el("h3", "세이크팜 클래식 떡크노믹스"),
                el("p", "토큰 전송 시 10% 떼감 -> 9%는 홀더들한테 떡돌림, 1%는 떡방앗간에 팁으로 제공 (팁은 이벤트, 에드, 기부, 개발자 사리사욕에 쓰임)"),
                el("p", "세이크팜 클래식은 클레이튼 밈 토큰입니다. 따라서 클레이튼 지갑인 카이카스 지갑이 필요합니다."),
                el("a", "카이카스 지갑 다운로드", {
                ...
                el("p.warning", "절대 본인의 세이크팜 클래식을 본인의 지갑에 전송하지 마세요. 세이크팜 클래식을 모두 잃어버릴 수 있습니다!"),
                el(".card",
                    el("h5", "세이크팜 클래식 가격"),
                    el("h6", (this.priceDisplay = el("span.price", "...")), " KLAY\n"),
                ),
                el("h3", "클레이로 세이크팜 클래식 사기"),
                el("p", "세이크팜 클래식을 살때도 떡크노믹스 때문에 10%를 적게 받습니다."),
                el(".form",
                ...
                el("h3", "세이크팜 클래식 클레이로 팔기"),
                el("p", "세이크팜 클래식을 펄때도 떡크노믹스 때문에 10%를 적게 받습니다."),
                el(".form",
